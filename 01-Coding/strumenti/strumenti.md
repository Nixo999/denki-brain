...
- `liste-bilanci/` — liste di aziende per **codice ATECO e fatturato**, col telefono verificato: companyreports per il bilancio, Pagine Gialle per il numero (nome identico e stesso comune), distanza da Seveso sull'indirizzo. Due giri, ATECO → telefono (`parallelo.py`) e categoria Pagine Gialle → bilancio (`pg_categorie.py`, `abbina.py`), poi `unisci.py` ed `excel.py` (`unisci.py Facchinaggio finale_f.json` + `excel_f.py` per la lista di un settore solo). Scrivono cache accanto a sé: **si copiano in una cartella di lavoro fuori dal vault** e si lanciano da lì. Usi → [[2026-09-25-opero-facchinaggio-allestimento]], [[2026-09-25-opero-facchinaggio]]
