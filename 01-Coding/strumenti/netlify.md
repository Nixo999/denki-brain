...
| [[sito-leibeautyroom]] | Lei Beauty Room, centro estetico di Tania, Torino Cavoretto — Instagram `@lei_beauty_room_`, ha risposto «Grazie manda pure qui» e aspetta la bozza nel DM | ✅ **online su <https://leibeautyroom.netlify.app>** dal 24 settembre 2026, progetto `leibeautyroom` sul team `nicola-la-rezza`, deploy dal CLI `--prod --no-build`, repo `Nixo999/leibeautyroom-site` privata; **tre sbarramenti verificati con `curl` sul deploy pubblicato**, i file di lavoro (`BRIEF.md`, `MONDO.md`, `PRODUCT.md`, `MONDI.md`, `LEGGIMI.md`, `.impeccable/`, `.claude/`) rispondono 404 |
| [[sito-perunpelo]] | Per un Pelo, toelettatura di Ambra Longoni, Nembro (BG) — Instagram `@toelettatura.perunpelo`, ha risposto «si me la mandi pure» e aspetta la bozza nel DM | ✅ **online su <https://perunpelo.netlify.app>** dal 25 settembre 2026, progetto `perunpelo` sul team `nicola-la-rezza` (id `a753a992-cb09-4c78-9c7a-9b066b4c40eb`), deploy dal CLI `--prod --no-build`, repo `Nixo999/perunpelo-site` privata; **tre sbarramenti verificati con `curl` sul deploy pubblicato**, RACCOLTA/MONDI/MONDO/PRODUCT e `sorgenti/` rispondono 404, badge «Powered by Netlify» spento dall'API |
